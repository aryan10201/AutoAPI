...
queue_lock = Lock()
requests_lock = Lock()
file_lock = Lock()  # lock to control access to the output file
...
# We'll keep the file handle open globally
file_handle = None
...
def write_name_to_file(name):
    """
    Write a single name to the output file, ensuring thread safety,
    and flush the file so changes appear in real-time.
    """
    global file_handle
    ...
    with names_lock:
        for name in results:
            if name not in found_names:
                ...
                # Write newly found names to file immediately
...
def explore_names():
    ...
    Writes found names to the output file in real-time.
    """
    global file_handle
    ...
    # Open the file for writing right from the start
