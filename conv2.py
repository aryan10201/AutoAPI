...
class AutocompleteAPIClient:
    ...
    def get_suggestions(self, query: str) -> List[str]:
        url = f"{self.base_url}/v2/autocomplete"
        params = {"query": query}
...
class V2Extractor:
    ...
    def _save_results(self):
        results_path = os.path.join(self.results_dir, "v2_names.json")
...
def main(results_dir: str):
    extractor = V2Extractor(results_dir)
    extractor.extract_names()

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Run V2 name extractor')
